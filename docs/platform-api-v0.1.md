...
本文档定义平台最小控制面 API：身份、目录、模板下发、token、请求事件、指标。  
...
```json
{
  ...
  "buyer_id": "user_01htz0demo",
  "role_scopes": ["buyer"],
  "api_key": "sk_live_once_only_xxx",
  "api_key_fingerprint": "akf_2db51f87",
...
- 验签材料：`seller_public_key`（公钥轮换窗口可返回 `seller_public_keys[]`）
- 合约构建入口：`template_ref`
...
- 校验失败返回：`AUTH_SCOPE_FORBIDDEN` 或 `AUTH_RESOURCE_FORBIDDEN`。
