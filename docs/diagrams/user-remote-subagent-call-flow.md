...
- `GET /v1/catalog/subagents`（或未来 `GET /v1/catalog/search`）需覆盖：
  ...
  - 安全验收：`seller_public_key`（或轮换窗口的 `seller_public_keys[]`）
  - 合约构建入口：`template_ref`
...
- `Controller` 与通信通道之间建议只依赖统一的 `Transport Adapter` 接口。
- Adapter 可有多种实现：`Email MCP`、`SMTP/API 自动发邮件工具`、`官方 HTTP/Webhook 通道`。
- 因此并不是 Agent 直接调 MCP；Agent 只和 Controller 交互，Controller 再调用具体通信工具。
...
sequenceDiagram
    ...
    participant BTA as Buyer Mail Transport Adapter
    participant EC as Email Channel Platform
    participant STA as Seller Mail Transport Adapter
    ...
    alt Search（增强预留）
        ...
        alt Catalog失败
            ...
            P-->>BC: [B2-RES] 候选列表 + capabilities/supported_task_types/sla/pricing/template_ref/seller_public_key
        end
    end
    ...
    BC->>BTA: [E1-REQ] 发送任务邮件 [CROC][TASK][request_id]
    alt 发信失败且重试耗尽
