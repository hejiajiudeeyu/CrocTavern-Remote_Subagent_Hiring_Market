...
- **超时确认**：`soft_timeout` 默认先询问是否继续等待，`hard_timeout` 未确认继续则终态 `TIMED_OUT`。
...
| 方法 | 路径 | 核心用途 |
| :--- | :--- | :--- |
| `POST` | `/v1/users/register` | 注册用户主体并签发 API Key（默认 buyer scope） |
| `GET` | `/v1/catalog/subagents` | 买家检索可用 subagent |
| `POST` | `/v1/catalog/subagents` | 注册/更新目录项（MVP 阶段倾向手工导入） |
| `GET` | `/v1/catalog/subagents/{subagent_id}/template-bundle` | 拉取能力模板包，构造合约输入输出 |
| `POST` | `/v1/tokens/task` | 买家申请短期授权任务 Token |
| `POST` | `/v1/tokens/introspect` | 卖家在线查验 Token 合法性 |
| `POST` | `/v1/requests/{request_id}/delivery-meta` | 按请求获取投递地址与线程策略 |
...
3. **投递信息准备**：买家向平台申请 `delivery-meta`，获得本次请求专用投递地址与线程策略。
4. **任务派发**：通过邮箱 MCP 发送任务邮件。
5. **安全校验**：卖家读取邮件，校验合约、Token 以及自身的护栏策略。
6. **确认接单**：卖家通过服务端发送 ACK（声明已接单，并可附带预期 ETA）。
7. **执行与回传**：卖家执行完毕后，回传结构化的结果包（同邮件线程回复）。
8. **结果验收**：买家轮询请求事件+邮箱收件箱，进行签名与 Schema 验证（包含错误结果包校验）。
9. **指标上报**：买卖双方各自向平台上报最小指标以供后续的评测展示。
...
- [docs/diagrams/README.md](docs/diagrams/README.md) - 时序图与权限图索引
